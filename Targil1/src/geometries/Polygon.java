...
public class Polygon implements Geometry {
	...
	public List<Point3D> findIntsersections(Ray ray) {
		...
		if(ray.getP0().equals(plane.getQ0())) return null;
			...
			if(t==0) return null;
			if(plane.getNormal().dotProduct(ray.getDir())!=0)
				{
				...
				for(int i=0;i<vertices.size()-2;i++)
				{
					...
					if(!isZero(ray.getDir().dotProduct(v2.crossProduct(v1))) &&ray.getDir().dotProduct(v1.crossProduct(v0))/ray.getDir().dotProduct(v2.crossProduct(v1))<0)return null;
				}
