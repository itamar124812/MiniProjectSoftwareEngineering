...
public class ViewPlaneTests {
	private int CountIntersection(Camera camera,Intersectable intersectable,int Nx, int Ny) {
		int count=0;
		List<Point3D> temp;
      for(int i=0;	i<Nx;	i++)
    	  for(int j=0;	j<Ny; j++) {
	...
	public void testConstructRayThroughPixelWithSphere() {
		...
		Camera camera = new Camera(Point3D.ZERO, new Vector(0, 0, -1),
		 ...
		 assertEquals("Wrong number of points", 18, CountIntersection(camera,sphere,3, 3));
		 ...
		 assertEquals("Wrong number of points", 10, CountIntersection(camera,sphere,3, 3));
		 ...
		 assertEquals("Wrong number of points", 9, CountIntersection(camera,sphere,3, 3));
			// TC05: The sphere behind the view plane
		 camera = new Camera(Point3D.ZERO, new Vector(0, 0, -1), new Vector(0, 1, 0))
				 .setDistance(1).setViewPlaneSize(3, 3);
		  sphere = new Sphere(new Point3D(0,0,1),0.5d);
		 assertEquals("Wrong number of points", 0, CountIntersection(camera,sphere,3, 3));
	}
	...
	public void testConstructRayThroughPixelWithPlane() {
		...
		assertEquals("Wrong number of points", 9, CountIntersection(camera,plane,3, 3));
		...
		assertEquals("Wrong number of points", 6, CountIntersection(camera,plane,3, 3));
	}
	...
	public void testConstructRayThroughPixelWithTriangle() {
		...
		assertEquals("Wrong number of points", 1, CountIntersection(camera,triangle,3, 3));
			// TC02: The triangle cut by two points of the view plane
		camera = new Camera(Point3D.ZERO, new Vector(0, 0, -1), 
				new Vector(0, 1, 0)).setDistance(1).setViewPlaneSize(3, 3);
		   triangle=new Triangle(new Point3D(0, 20, -2),new Point3D(-1, -1, -2),new Point3D(1, -1, -2));
		 assertEquals("Wrong number of points", 2, CountIntersection(camera,triangle,3, 3));
